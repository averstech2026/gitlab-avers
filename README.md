...
6. Метка проекта на карточке Planka (`pr:Front` и т.п.): когда в GitLab появляется связанный MR из репо (например `avers/front` с `Closes avers/AVERS#N`) или system note об MR.
7. Уже лежащие в колонке карточки при старте bridge **не** трогаются (только новые переносы).
